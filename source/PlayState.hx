...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'self-insert' | 'jake':
					curStage = 'jake';
				case 'psycopathic':
					curStage = 'jakeTainted';
		...
		{
			case 'jake':
				var bg:BGSprite = new BGSprite('jake/sky', -600, -200, 0.9, 0.9);
				add(bg);

				if(!ClientPrefs.lowQuality) {
					var city:BGSprite = new BGSprite('jake/city', -500, -300, 1.3, 1.3);
					city.setGraphicSize(Std.int(city.width * 0.9));
					city.updateHitbox();
					add(city);
				}

				var street:BGSprite = new BGSprite('jake/street', -650, 600, 0.9, 0.9);
				street.setGraphicSize(Std.int(street.width * 1.1));
				street.updateHitbox();
				add(street);

			case 'jakeTainted':
				var bg:BGSprite = new BGSprite('jake-tainted/sky', -600, -200, 0.9, 0.9);
				add(bg);

				if(!ClientPrefs.lowQuality) {
					var city:BGSprite = new BGSprite('jake-tainted/city', -500, -300, 1.3, 1.3);
					city.setGraphicSize(Std.int(city.width * 0.9));
					city.updateHitbox();
					add(city);
				}

				var street:BGSprite = new BGSprite('jake-tainted/street', -650, 600, 0.9, 0.9);
				street.setGraphicSize(Std.int(street.width * 1.1));
				street.updateHitbox();
				add(street);
